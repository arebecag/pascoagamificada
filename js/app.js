...
function setAuthenticatedUser(email) {
  localStorage.setItem(AUTH_KEY, normalizeEmail(email));
...
function initAuthGate() {
  ...
  const storedEmail = normalizeEmail(localStorage.getItem(AUTH_KEY));
  if (storedEmail && isAuthorizedEmail(storedEmail)) {
    setAuthenticatedUser(storedEmail);
    bootDashboard();
    return;
  }

  if (!loginGate || !loginForm || !loginEmail) {
    bootDashboard();
    return;
  }
  ...
  loginForm.addEventListener('submit', (event) => {
    ...
    if (!isAuthorizedEmail(email)) {
      if (loginError) {
        loginError.textContent = 'E-mail não autorizado. Solicite liberação ao time responsável.';
      }
      return;
    }
